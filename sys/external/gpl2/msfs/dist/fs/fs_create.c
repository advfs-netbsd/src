...
{              
    ...
    for (qtype = 0; qtype < MAXQUOTAS; qtype++) {
        ...
        if (((qtype == USRQUOTA) ||
            ...
            MIGTRUNC_LOCK_READ( &(fsNp->qi[qtype].qiAccessp->xtnts) );
    ...
    for (qtype = 0; qtype < MAXQUOTAS; qtype++) {
        if (quota_mig_trunc_lock[qtype]) {
            MIGTRUNC_UNLOCK( &(fsNp->qi[qtype].qiAccessp->xtnts) );
            quota_mig_trunc_lock[qtype] = FALSE;
        }
    }
                ...
                )
{
    ...
    if (ssp->fs_flag & (MOD_MTIME | MOD_ATIME | MOD_CTIME)) {
        ...
        if (ssp->fs_flag & MOD_ATIME) {
            ...
            ssp->dir_stats.advfs_st_uatime = (new_time.tv_usec * 1000);
        }
        if (ssp->fs_flag & MOD_CTIME) {
            ssp->dir_stats.advfs_st_ctime = new_time.tv_sec;
            ssp->dir_stats.advfs_st_uctime = (new_time.tv_usec * 1000);
        }
